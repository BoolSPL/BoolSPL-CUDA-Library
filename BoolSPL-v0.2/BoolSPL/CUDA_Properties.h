...
//	pArgc = &argc;
//	pArgv = argv;
	printf("\n   --- Extended General Information for device ---   \n");
	printf("\n Starting...\n\n"); //printf("%s Starting...\n\n", argv[0]);
	printf(
...
void BoolSPLGMinimalRequires()
{
	...
	for (int i = 0; i < devCount; ++i)
	{
		...
		printf("\nMinimal requires compute capability 3.0 or later to run the Library BoolSPLG;\n");
...
void bandwidthTest(const unsigned int bytes, int nElements)
{
	...
	profileCopies(h_aPageable, h_bPageable, d_a, nElements, " PAGEABLE Memory");
	profileCopies(h_aPinned, h_bPinned, d_a, nElements, " PINNED Memory");
